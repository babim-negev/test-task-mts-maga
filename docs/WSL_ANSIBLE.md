...
```bash
sudo apt update
sudo apt install -y git python3 python3-venv python3-pip openssh-client sshpass
```

`sshpass` не обязателен для финального сценария, но удобен, если сначала надо проверить парольный SSH. Рекомендуемый итоговый вариант - SSH key + passwordless sudo.

## 3. Подготовить SSH key
