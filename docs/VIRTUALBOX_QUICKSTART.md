...
- сеть будет настроена на следующем шаге.
...
**ЕСЛИ СЕТЬ В VM НЕ РАБОТАЕТ, В НАСТРОЙКАХ АДАПТЕРА УСТАНОВИТЕ «НЕРАЗБОРЧИВЫЙ РЕЖИМ — РАЗРЕШИТЬ ВСЕ».**
...
### ЕСЛИ ВЫ ЗАПУСКАЕТЕ ДВЕ ВИРТУАЛЬНЫЕ МАШИНЫ

Для связи двух VM между собой и доступа в интернет настройте сетевые адаптеры
на **каждой** виртуальной машине:

- `Адаптер 1`: включите адаптер и выберите `NAT`;
- `Адаптер 2`: включите адаптер и выберите `Виртуальный адаптер хоста`;
- в поле `Имя` выберите `VirtualBox Host-Only Ethernet Adapter` (обычно он один);
- нажмите кнопку обновления рядом с полем `MAC-адрес`, чтобы сгенерировать новый
  MAC-адрес.

Повторите эту настройку для второй VM и **ОБЯЗАТЕЛЬНО ОБНОВИТЕ MAC-АДРЕС**:
у двух виртуальных машин MAC-адреса не должны совпадать.
